...
class PHPMailer {
   ...
   * @var string
   */
  public $conntentType       = 'text/plain';
 ...
 */
  ...
  public function SmtpConnect() {
    ...
    $index = 0;
    $connnection = $this->smtp->Connected();

    // Retry while there is no connection
    try {
      while($index < count($hosts) && !$connnection) {
        $hostinfo = array();
  ...
  */
  ...
  public function CreateHeader() {
    ...
    if(isset($this->ConfirmReadingTo) && !empty($this->ConfirmReadingTo)) {
   ...
   * @return string
   */
  protected function GetBoundary($boundary, $charSet, $conntentType, $encoding) {
    $result = '';
    if($charSet == '') {
      $charSet = $this->CharSet;
    }
    if($conntentType == '') {
      $conntentType = $this->ContentType;
    }
    if($encoding == '') {
      $encoding = $this->Encoding;
    }
    $result .= $this->TextLine('--' . $boundary);
    $result .= sprintf("Content-Type: %s; charset=%s", $conntentType, $charSet);
    $result .= $this->LE;
